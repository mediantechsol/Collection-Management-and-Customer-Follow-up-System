...
import type { CurrencyDistributionItem } from '@/types/models';
...
const CURRENCY_COLORS: Record<string, { bg: string; border: string }> = {
  YER: { bg: '#10B981', border: '#059669' }, // زمردي
  USD: { bg: '#3B82F6', border: '#2563EB' }, // أزرق
  SAR: { bg: '#F59E0B', border: '#D97706' }, // كهرماني
};

export function CurrencyDistributionChart({ data, loading }: Props) {
  ...
  const totalYer = data.reduce((s, it) => s + (it.amount_yer || 0), 0);

  let cumulativePercent = 0;
  const slices = data
    .filter((d) => d.amount_yer > 0 || d.percentage > 0)
    .map((item, idx) => {
      const p = totalYer > 0 ? item.amount_yer / totalYer : 1 / data.length;
      ...
      return {
        ...
        startPercent,
        endPercent,
        color: CURRENCY_COLORS[item.currency] ?? { bg: '#64748B', border: '#475569' },
      };
    });
    ...
    <div className="card flex flex-col justify-between p-5">
      <div className="flex items-center justify-between border-b border-gray-100 pb-3">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-bold text-gray-900">توزيع المديونيات حسب العملة</h3>
        </div>
        <span className="text-xs text-gray-500 font-medium">YER / USD / SAR</span>
      </div>
      ...
      <div className="my-3 flex flex-col items-center justify-center sm:flex-row sm:gap-6">
        ...
        <div className="relative flex h-[180px] w-[180px] shrink-0 items-center justify-center">
          ...
          <div className="pointer-events-none absolute flex flex-col items-center justify-center text-center">
            {activeItem ? (
              <>
                <span className="text-xs font-bold text-gray-500">{activeItem.currency}</span>
              ...
              <>
                ...
                <span className="text-xs font-bold text-gray-800 mono">{fmt(totalYer)}</span>
        ...
        <div className="mt-3 flex flex-1 flex-col justify-center gap-2 sm:mt-0">
          {data.map((item, idx) => {
            const c = CURRENCY_COLORS[item.currency] ?? { bg: '#64748B', border: '#475569' };
            ...
            return (
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  />
                  <div>
                    <div className="text-xs font-bold text-gray-800">{item.currency_name}</div>
                    <div className="text-[11px] text-gray-400 mono">
                      {fmt(item.amount_original)} {item.currency}
                    </div>
                  </div>
                </div>
                <div className="text-left">
                  <span className="text-xs font-bold text-gray-900 mono">{item.percentage}%</span>
                  <div className="text-[10.5px] text-gray-400 mono">{fmt(item.amount_yer)} ر.ي</div>
                </div>
              </div>
            );
          })}
        ...
        {activeItem ? (
          <>
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-gray-800">{activeItem.currency_name}:</span>
              <span className="text-gray-600 mono">{fmt(activeItem.amount_original)} {activeItem.currency}</span>
            </div>
            <div className="flex items-center gap-2">
              <span className="text-gray-400">المعادل:</span>
              <span className="font-bold text-gray-900 mono">{fmt(activeItem.amount_yer)} ر.ي</span>
          ...
          <span className="w-full text-center text-gray-400 text-[11.5px]">
            مرر مؤشر الماوس فوق أي شريحة عملة لعرض التفاصيل
          </span>
        )}
      </div>
    </div>
